###########################################################################################
# Author: Danni Yu
# Original Date: 2021-3-22
# Last Modified Date: 2022-08-31
# Description: convert mockup shells into an integrated RTF file
###########################################################################################
  
## To debug:  loa[i,]
## Alert: before running this code, check the loa table, the order must be T->L->F

# paths, files, ...
if(TRUE){
  rm(list=ls())
  who.maintain<-"Danni Yu <dyu@nektar.com>"
  
  #Rscirpt for CSR mockup shells generation
  dir(projpath<-getwd())
  
  dir(progpath<-projpath)
  progfile<-'CSR_safety_shell.R'

  loafile <- mockfile<- mockfileF<- "CSR_safety_LoA.xlsx"
...
print( paste('For questions, please consult', who.maintain))
